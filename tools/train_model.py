...
from learning_data import load_dataset, FREQUENCY, PREPROCESSING_VERSION
from nmr_lab import ARCHITECTURES, build_model, group_split
...
def main():
    ...
    try:
        data = load_dataset(args.data, training=True)
        contract = data["contract"]
        features, labels = data["features"], data["P"][:, None]
        group_column = contract["group_column"]
        groups = data["identifiers"][group_column]
    ...
    torch.save({"architecture": args.architecture, "model_state": best_state,
                "simulator": data["simulator"], "input_contract": contract},
               args.output_dir / "model.pt")
    np.savez(args.output_dir / "scaler.npz", feature_mean=mean, feature_std=std, target_scale=np.array([target_scale]),
             frequency_mhz=FREQUENCY, feature_mode=contract["feature_mode"], voltage_unit=contract["voltage_unit"],
             preprocessing_version=PREPROCESSING_VERSION)
    config = {key: str(value) if isinstance(value, Path) else value for key, value in vars(args).items()}
    config["simulator"] = data["simulator"]
    config["input_contract"] = contract
    config["dataset_sha256"] = digest(args.data)
    (args.output_dir / "config.json").write_text(json.dumps(config, indent=2))
    split = {"strategy": "grouped", "group_column": group_column, "rows": {}, "groups": {}, "group_values": {}}
    ...
    for name, idx, loader in (("validation", validation, loaders[1]), ("test", test, loaders[2])):
        ...
        columns = [truth, prediction, truth-prediction, data["identifiers"]["configuration_id"][idx]]
        header = "P_true,P_pred,P_residual,configuration_id"
        if group_column != "configuration_id":
            columns.append(groups[idx])
            header += ","+group_column
        np.savetxt(args.output_dir / f"{name}_predictions.csv", np.column_stack(columns),
    ...
    provenance = {
        ...
        "code_sha256": {name: digest(Path(__file__).parent/name) for name in
                        ("nmr_lab.py", "learning_data.py", "baseline_data.py", "train_model.py", "lineshape.py", "circuit.py", "analyze_predictions.py")},
