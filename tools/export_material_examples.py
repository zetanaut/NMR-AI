...
<div class="layout"><aside class="sidebar"><p class="eyebrow">Three experimental examples</p><nav aria-label="Experimental examples"><a href="walkthrough.html"><strong>Run the tutorial step by step →</strong></a><a href="matching.html">1 · Single-site starting model</a><a href="butanol.html">2 · Butanol: C–D and O–D</a><a href="uva-nd3.html">3 · UVA-ND3 data</a><a href="#model">Model and assumptions</a><a href="#results">Fits and full residuals</a><a href="#reproduce">Reproduce the example</a><a href="#limits">Interpretation and next steps</a><a href="baseline.html">Physical-baseline practical</a><a href="index.html#network">500-bin learning models</a></nav><p class="sidebar-footer">All examples use 500 bins. Match the material and readout.</p></aside>
<main id="main"><section class="hero"><p class="eyebrow">Experimental example {number} of 3</p><h1>{title}</h1><p class="lede">{lede}</p><aside class="note"><h3>Run this example in sequence</h3><p><a href="walkthrough.html#{"butanol" if number == 2 else "nd3"}">Walkthrough step {number+3}</a> gives the command, expected output, and next action. Begin at step 1 for setup and input checks; use this page for the model and reference results.</p></aside></section>
{content}
...
<section id="reproduce" class="chapter"><h2>Run the comparison after the single-site fit.</h2><p>Complete <a href="walkthrough.html#single-site">walkthrough step 4</a>, then use its report here. Run from the repository root in the activated environment with the thread settings from step 1. Use fresh output paths.</p>
<pre><code>python tools/match_butanol.py \\
...
python tools/plot_fit.py --fit-dir local-results/walkthrough/butanol \\
  --output local-results/walkthrough/butanol-fits.png</code></pre>
<p><strong>Check:</strong> five scan summaries, a <code>butanol_report.json</code>, and five fit CSVs. Open <code>local-results/walkthrough/butanol-fits.png</code> to inspect raw data, the joint fit and baseline, the fitted response, and every raw-minus-fit residual. The terminal prints each RMS reduction against your single-site report. <strong>Next:</strong> <a href="walkthrough.html#nd3">fit UVA-ND3 in step 6</a>.</p>
...
<p>Every working array uses <strong>fⱼ = 32.3 + 0.0015287 j MHz, j = 0…499</strong>, ending at 33.0628213 MHz. Raw phase is linearly resampled from the original frequency coordinates in float64. The stored reference and its subtraction are excluded from this raw-phase fit. The reader automatically checks the resampling against the original measurements. Interpolation changes resolution and correlates errors.</p>
...
<section id="reproduce" class="chapter"><h2>Run the raw UVA-ND3 example.</h2><p>The 500-bin working file is included and checked in <a href="walkthrough.html#inspect">walkthrough step 2</a>. Continue in the activated environment using a fresh output directory.</p>
...
python tools/plot_fit.py --fit-dir local-results/walkthrough/nd3 \\
  --output local-results/walkthrough/nd3-fits.png</code></pre>
<p><strong>Check:</strong> five record summaries, a <code>uva_nd3_report.json</code>, and five CSVs. Open <code>local-results/walkthrough/nd3-fits.png</code>; its first column retains the baseline in each raw input. Inspect the bound flags, including the reference run's 2000 pF limit for record 376. <strong>Next:</strong> <a href="walkthrough.html#generate">return to the single-site configurations to generate learning data in step 7</a>.</p>
