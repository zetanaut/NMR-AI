...
<pre><code>python tools/train_model.py --data local-results/experiment-anchored-500-v2.npz \\
...
python tools/predict.py --model-dir local-results/my-lineshape-model \\
  ...
  --output local-results/my-lineshape-test.csv</code></pre>
...
<p>To reproduce this declared run with deferred test evaluation, use <code>python tools/run_model_comparison.py --protocol configs/lineshape-training.json</code>, then <code>python tools/export_lineshape_training.py</code>. Use matching fresh <code>--runs-dir</code> paths for repeated runs.</p>
